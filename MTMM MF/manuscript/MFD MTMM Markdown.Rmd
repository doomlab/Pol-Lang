...
##preprocess only works on one string at a time, so going to use a loop go through all the rows 
##writing columns are 27, 29, 30, 32, 34 - must make these characters first
finalsample$Q27 = as.character(finalsample$Q27)
finalsample$Q29 = as.character(finalsample$Q29)
finalsample$Q30 = as.character(finalsample$Q30)
finalsample$Q32 = as.character(finalsample$Q32)
finalsample$Q34 = as.character(finalsample$Q34)

for (i in 1:nrow(finalsample)) {
  
  finalsample$Q27[i] = preprocess(finalsample$Q27[i], case="lower", remove.punct=TRUE)
  finalsample$Q29[i] = preprocess(finalsample$Q29[i], case="lower", remove.punct=TRUE)
  finalsample$Q30[i] = preprocess(finalsample$Q30[i], case="lower", remove.punct=TRUE)
  finalsample$Q32[i] = preprocess(finalsample$Q32[i], case="lower", remove.punct=TRUE)
  finalsample$Q34[i] = preprocess(finalsample$Q34[i], case="lower", remove.punct=TRUE)
...
#replacepeople_demo1 = subset(finalsample, missing <= 5)  #we are using 5%, yes? 
#dontpeople_demo1 = subset(finalsample, missing > 5)
#apply(replacepeople, 2, percentmiss) 

##be sure to exclude the writing columns columns 2:6 
#I take it I should also remove column 1: ResponseId? 
##outliers
mahal1 = mahalanobis(finalsample[ , -c(1:6)], 
                    colMeans(finalsample[ , -c(1:6)], na.rm = TRUE),
                    cov(finalsample[ , -c(1:6)], use="pairwise.complete.obs"))
mahal1
cutoff1 = qchisq(1 - .001,ncol(finalsample[ , -c(1:6)]))
cutoff1
ncol(finalsample[ , -c(1:6)])
summary(mahal1 < cutoff) #Hmm what to do w/ NAs?? -WP
noout1 = subset(finalsample, mahal1 < cutoff)

##assumptions
#correlations for MFQ
correlations1 = cor(noout1[,-c(1:6)], use="pairwise.complete.obs")
correlations1
symnum(correlations1)

#making the random stuff
random1 = rchisq(nrow(noout1), 7)
fake1 = lm(random~., data=noout1)
#normal
standardized1 = rstudent(fake1)
hist(standardized1, breaks=15) #ugh, look at the error we get for this one...
#linear
{qqnorm(standardized1) #...and this one...
abline(0,1)}
#homog + s
...
fullsample = merge(sample1, sample2, all = T)
...
summary(mahal2 < cutoff) #Hmm what to do w/ NAs?? 
noout2 = subset(finalsample, mahal2 < cutoff)
...
fake2 = lm(random2~., data=noout2)

#normal
standardized2 = rstudent(fake2)
hist(standardized2, breaks=15) # The errors are making me sad :(  
#linear
{qqnorm(standardized2)   
abline(0,1)}    #Oy vey, these errors!
#homog + s
